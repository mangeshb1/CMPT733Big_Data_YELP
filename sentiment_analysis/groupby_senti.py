...
conf = SparkConf().setAppName("IBM Watson Sentimemnt Analysis Group By")
sc = SparkContext(conf=conf)
sqlContext = SQLContext(sc)

#read parquet
if (len(sys.argv) != 4):
    print "usuage: loc output"
    raise SystemExit
...
#combine both dfs
senti_df = sqlContext.parquetFile(loc1)
senti_df2 = sqlContext.parquetFile(loc2)

senti_df.registerTempTable("senti_df")
senti_df2.insertInto("senti_df")
...
#perform aggregates
senti_agg = senti_df.select(senti_df.lv_id, "anger", "disgust", "fear", \
                                "joy", "sadness", "agreeness", "emotion")

senti_agg = senti_agg.groupby(senti_df.lv_id.alias('lv_id2')).avg().cache()
...
out_df = senti_df.join(senti_agg, senti_agg.lv_id2==senti_df.lv_id)
#print ("TOTAL COUNT: ", out_df.count())

out_df = out_df.select(senti_df.lv_id, out_df.yelp_id, out_df.name, \
                       out_df.vio_per_year, out_df.four_scale_stars, \
                       out_df.five_scale_stars, \
